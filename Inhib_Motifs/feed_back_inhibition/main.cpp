...
void render() {
  ...
  if ( pause_animation == 0 ) {
      for (int i = 0; i < DTS_PER_RENDER; ++i) {   //Each pass through the loop is one render every DTS_PER_RENDER DTs
        ...
        synapseKernelLauncher_a (syn_stream[6], syn_b2s1, A_cellLayer2, B_cellLayer2, syn_A2_B2, weights_b1, SMALL, B2S, syn_switches1, RADIUS );   //B2S takes the postsynaptic array size
        synapseKernelLauncher_a (syn_stream[7], syn_b2s2, A_cellLayer3, B_cellLayer3, syn_A3_B3, weights_b2, SMALL, B2S, syn_switches1, RADIUS );   //B2S takes the postsynaptic array size
        synapseKernelLauncher_a (syn_stream[8], syn_b2s3, A_cellLayer4, B_cellLayer4, syn_A4_B4, weights_b3, SMALL, B2S, syn_switches1, RADIUS );   //B2S takes the postsynaptic array size
        synapseKernelLauncher_a (syn_stream[9], syn_b2s4, A_cellLayer5, B_cellLayer5, syn_A5_B5, weights_b4, SMALL, B2S, syn_switches1, RADIUS );   //B2S takes the postsynaptic array size
																		    //
        synapseKernelLauncher_a (syn_stream[10],syn_s2b1, B_cellLayer2, A_cellLayer1, syn_B1_A2, weights_c1, SMALL, S2B, syn_switches1, RADIUS );   //S2B takes the presynaptic array size
        synapseKernelLauncher_a (syn_stream[11],syn_s2b2, B_cellLayer3, A_cellLayer2, syn_B2_A3, weights_c2, SMALL, S2B, syn_switches1, RADIUS );   //S2B takes the presynaptic array size
        synapseKernelLauncher_a (syn_stream[12],syn_s2b3, B_cellLayer4, A_cellLayer3, syn_B3_A4, weights_c3, SMALL, S2B, syn_switches1, RADIUS );   //S2B takes the presynaptic array size
        synapseKernelLauncher_a (syn_stream[13],syn_s2b4, B_cellLayer5, A_cellLayer4, syn_B4_A5, weights_c4, SMALL, S2B, syn_switches1, RADIUS );   //S2B takes the presynaptic array size
	...
	//sync so that all the synapse kernel launches are forced to complete before cell kernels start to launch
        ...
        cellKernelLauncher_aelif(cell_stream[6], B_layers[1].vm,   B_cellLayer2, SMALL,  cell_switches1,  layer1_params ); 
        cellKernelLauncher_aelif(cell_stream[7], B_layers[2].vm,   B_cellLayer3, SMALL,  cell_switches1,  layer1_params ); 
        cellKernelLauncher_aelif(cell_stream[8], B_layers[3].vm,   B_cellLayer4, SMALL,  cell_switches1,  layer1_params ); 
        cellKernelLauncher_aelif(cell_stream[9], B_layers[4].vm,   B_cellLayer5, SMALL,  cell_switches1,  layer1_params ); 
...
int main(int argc, char** argv) {
  ...
  //S2B layer has one synaptic current for each presynaptic cell in SMALL layer, which get fanned out to the post-synaptic BIG cell array
  cudaMalloc((void **)&syn_s2b1 , SMALL*SMALL *sizeof( float )); 
  cudaMalloc((void **)&syn_s2b2 , SMALL*SMALL *sizeof( float )); 
  cudaMalloc((void **)&syn_s2b3 , SMALL*SMALL *sizeof( float )); 
  cudaMalloc((void **)&syn_s2b4 , SMALL*SMALL *sizeof( float )); 
