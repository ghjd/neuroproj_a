...
void render() {
  ...
  if ( pause_animation == 0 ) {
      for (int i = 0; i < DTS_PER_RENDER; ++i) {   //Each pass through the loop is one render every DTS_PER_RENDER DTs
        ...
        synapseKernelLauncher_a (syn_stream[11],syn_s2b1, B_cellLayer1, A_cellLayer1, syn_B1_A1, weights_c1, SMALL, S2B, syn_switches1, RADIUS );   //S2B takes the presynaptic array size
        ...
        synapseKernelLauncher_a (syn_stream[15],syn_s2s1, B_cellLayer5, B_cellLayer1, syn_B5_B1, weights_d1, SMALL, S2S, syn_switches1, RADIUS );   //S2B takes the presynaptic array size
	...
	//sync so that all the synapse kernel launches are forced to complete before cell kernels start to launch
        ...
        cellKernelLauncher_aelif(cell_stream[6], B_layers[1].vm,   B_cellLayer1, SMALL,  cell_switches_sa,  layer1_params );  //this is the spontaneously active layer that inhibits A_layers[1]
...
int main(int argc, char** argv) {
  ...
  cell_switches_sa.en_spontaneous_activation = true;
  ...
  cudaMalloc((void **)&syn_B1_A1, BIG*BIG*    sizeof( synapse_data_type ));  
  ...
  cudaMalloc((void **)&syn_B5_B1, SMALL*SMALL*    sizeof( synapse_data_type ));  
  ...
  cudaMalloc((void **)&syn_s2b5 , SMALL*SMALL *sizeof( float )); 

  cudaMalloc((void **)&syn_s2s1 , SMALL*SMALL* (2*RADIUS+1) * (2*RADIUS+1) *sizeof( float )); //S2S 
  ...
  cudaMalloc((void **)&weights_d1 , (2*RADIUS+1) * (2*RADIUS+1) *sizeof( float ));
