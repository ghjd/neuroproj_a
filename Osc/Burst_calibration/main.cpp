...
void render() {
  ...
  */
  ...
  if ( pause_animation == 0 ) {
	...
	//support three sine waves with independent frequency, phase, offset for each cell group
	SimTime += dT;
	cell_switchesi.sin_1 = (sin (SimTime * Freq_1 * 2 * PI) + Offset_1) * Amplitude_1;
	cell_switchesi.sin_2 = (sin (SimTime * Freq_2 * 2 * PI) - 0.125) * Amplitude_2;
	...
	*/
	...
	//sync so that all the synapse kernel launches are forced to complete before cell kernels start to launch
        ...
        cellKernelLauncher_aelif(cell_stream[10], I_layer.vm,      I_cellLayer, BIG,   cell_switchesi,  layer1_params ); 
...
int main(int argc, char** argv) {
  ...
  cell_switchesi.en_input = 1;  //turn on automatically generated input in the input layer
  cell_switchesi.printf_cell = 44560;
  #include "switch.inc"
  cell_switchesi.en_bursting = 1;
  cell_switchesi.en_intrinsic_osc = 1;
  cell_switchesi.loc = {150, 150};

  cudaMalloc((void **)&I_cellLayer,   BIG*BIG*sizeof( lif_data_type ));
  cudaMalloc((void **)&J_cellLayer,   BIG*BIG*sizeof( lif_data_type ));
  ...
  cudaMalloc((void **)&B_cellLayer1,  BIG*BIG*sizeof( lif_data_type ));
  cudaMalloc((void **)&B_cellLayer2,  BIG*BIG*sizeof( lif_data_type ));
  cudaMalloc((void **)&B_cellLayer3,  BIG*BIG*sizeof( lif_data_type ));
  cudaMalloc((void **)&B_cellLayer4,  BIG*BIG*sizeof( lif_data_type ));
  cudaMalloc((void **)&B_cellLayer5,  BIG*BIG*sizeof( lif_data_type ));
  ...
  cudaMalloc((void **)&syn_I_B1,  BIG*BIG*sizeof( synapse_data_type ));   
  cudaMalloc((void **)&syn_B1_B2, BIG*BIG*sizeof( synapse_data_type ));  
  cudaMalloc((void **)&syn_B2_B3, BIG*BIG*sizeof( synapse_data_type ));  
  cudaMalloc((void **)&syn_B3_B4, BIG*BIG*sizeof( synapse_data_type ));  
  cudaMalloc((void **)&syn_B4_B5, BIG*BIG*sizeof( synapse_data_type ));  
  cudaMalloc((void **)&syn_B4_B5, BIG*BIG*sizeof( synapse_data_type ));  
  ...
  cudaMalloc((void **)&syn_b1 , BIG*BIG*(2*RADIUS+1) * (2*RADIUS+1) *sizeof( float ));
  cudaMalloc((void **)&syn_b2 , BIG*BIG*(2*RADIUS+1) * (2*RADIUS+1) *sizeof( float ));
  cudaMalloc((void **)&syn_b3 , BIG*BIG*(2*RADIUS+1) * (2*RADIUS+1) *sizeof( float ));
  cudaMalloc((void **)&syn_b4 , BIG*BIG*(2*RADIUS+1) * (2*RADIUS+1) *sizeof( float ));
  cudaMalloc((void **)&syn_b5 , BIG*BIG*(2*RADIUS+1) * (2*RADIUS+1) *sizeof( float ));
